...
class Parser:
    ...
    @classmethod
    def instructions_parser(self, filename):
    ...
    @classmethod
    def registers_parser(self, filename):
        file = open(filename, 'r')
        reg_count = 0
    ...
    @classmethod
    def configuration_parser(self, filename):
        file = open(filename, 'r')
        try:
            for line in file:
                ...
                if 'adder' in split_line[0]:
                    Parser._status_pipelining(FP_ADD, split_line[1])
                elif 'multiplier' in split_line[0]:
                    Parser._status_pipelining(FP_MUL, split_line[1])
                elif 'divider' in split_line[0]:
                    Parser._status_pipelining(FP_DIV, split_line[1])
    ...
    @classmethod
    def _status_pipelining(self, fu_hash, param_string):
