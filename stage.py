...
class InstFetchStage(Stage):
    def __init__(self, instruction):
        Stage.__init__(self, instruction)
        self.name = 'IF'
        self.cache_hit, self.cycles = InstCache.readcache(self.instruction.address)
    ...
    def next(self):
        ...
        if self.cycles <= 0 and STAGE['ID'] == FREE:
            STAGE['IF'] = FREE
            return InstDecodeStage(self.instruction)

        return self



class InstDecodeStage(Stage):
    def __init__(self, instruction):
        Stage.__init__(self, instruction)
        self.name = 'ID'
...
class ExecuteStage(Stage):
    ...
    def _bus_hazard(self):
        ...
        if self.instruction.name in ['LW', 'L.D', 'SW', 'S.D']:
            if not DataCache.if_hit(address) and STAGE['IBUS'] == BUSY:
                return True
        return False
...
class MemoryStage(ExecuteStage):
    ...
    def __init__(self, instruction):
        ...
        self.first_word_cycles, self.second_word_cycles = self._calculate_memory_cycles()
    ...
    def _calculate_memory_cycles(self):
        if self.instruction.name == 'LW':
            address = int(self.instruction.offset) + REGISTER[self.instruction.src_reg[0]]
            self.first_word_hit, REGISTER[self.instruction.dest_reg], cycles = DataCache.readcache(address)
            return cycles, 0

        elif self.instruction.name == 'L.D':
            address = int(self.instruction.offset) + REGISTER[self.instruction.src_reg[0]]
            self.first_word_hit, word, first_word_read_time = DataCache.readcache(address)
            self.second_word_hit, word, second_word_read_time = DataCache.readcache(address + 4)
            return first_word_read_time, second_word_read_time

        elif self.instruction.name == 'SW':
            address = int(self.instruction.offset) + REGISTER[self.instruction.src_reg[1]]
            self.first_word_hit, cycles = DataCache.writecache(address, REGISTER[self.instruction.src_reg[0]])
            return cycles, 0

        elif self.instruction.name == 'S.D':
            address = int(self.instruction.offset) + REGISTER[self.instruction.src_reg[1]]
            self.first_word_hit, first_word_write_time = DataCache.writecache(address, 0, False)
            self.second_word_hit, second_word_write_time = DataCache.writecache(address + 4, 0, False)
